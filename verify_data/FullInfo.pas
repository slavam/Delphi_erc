...
begin
  ...
  get_address_by_id_room_location(address_params, ADOq, current_owner_erc_code.id_room);

  get_periods_by_city(periods, ADOq, loading_params.main_city_id);
  LBPeriod.Items := periods;
  LBPeriod.ItemIndex := 0;
  LBPeriodClick(Sender);

  LAddress.Caption := 'Адрес: '+address_params.city_type+' '+ address_params.city_name;
...
(*
  ...
  if (pos(' пер.', address_params.street_name) > 0) or (pos(' пр.', address_params.street_name) > 0)
     or (pos(' пл.', address_params.street_name) > 0) or (pos(' б.', address_params.street_name) > 0)then
...
procedure TFormFullInfo.update_info;
var
...
begin
  ...
  get_debt_options_by_bankbook(debt_params, ADOQ, IntToStr(current_period_id),
  {Inttostr(bankbook_params.code_utility),} bankbook_params.id_bankbook_utilities);
...
//    IntToStr(current_period_id), bankbook_params.id_bankbook_utilities);
...
procedure TFormFullInfo.LBPeriodClick(Sender: TObject);
begin
