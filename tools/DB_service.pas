...
type
  TPaymentParams = class(TObject)
    id_bankbook_utility: integer;
    payment_date: Extended;
    send_date: Extended;
    sum: real;
    ward_name: string;
    filial: integer;
...
procedure get_payment_params(var l: TStringList; sp: TADOStoredProc;
  id_bankbook_utility: integer);
function active_period_name(q: TADOQuery; city_id: integer): string;
procedure update_templates_by_firm(q: TADOQuery; loading_params: TLoadingParams);
...
//      s := format('%d', [FieldByName('code_firm').AsInteger]);
//      l.AddObject(s, TObject(bb_params));
...
procedure crate_unconfirmed_period(q: TADOQuery; city_id: integer;
  report_date: Extended);
//  ; is_active_period: boolean);
var
  descr: string;
...
//        FieldByName('bank_book').AsString+'('+
//        FieldByName('full_name').AsString+')', TObject(bb_params));
        s := format('%4d-%12s-%s', [FieldByName('code_firm').AsInteger,
          FieldByName('bank_book').AsString,
          FieldByName('utility').AsString+'('+FieldByName('full_name').AsString+')']);
        TempList.AddObject(s, TObject(bb_params));
      next;
    end;
    close;
  end;
...
begin
  ...
  with q do begin
    SQL.Clear;
    SQL.Add('SELECT b.id id, h.id h_id, bu.id bu_id, b.code_firm code_firm, u.short_name utility, '+
      ' bu.code_utility code_utility, b.bank_book bank_book, ah.full_name full_name FROM bankbooks b '+
      ' JOIN humans h ON h.id = b.id_human AND h.id_room_location='+ IntToStr(payer_address_id)+
      ' JOIN bankbook_utilities bu ON bu.id_bankbook=b.id'+
      ' JOIN bankbook_attributes_debts d ON d.id_bankbook_utility=bu.id AND d.id_period='+IntToStr(period)+
      ' JOIN utilities u ON u.code=bu.code_utility ' +
      ' JOIN bankbook_attributes_humans ah ON ah.id_bankbook=b.id AND d.id_period '+
      ' between ah.id_period_begin and ah.id_period'+
      ' WHERE b.id_human !='+ IntToStr(payer_id)+
      ' ORDER BY bu.code_utility');
    Open;
    while not eof do begin
...
begin
  with sp do begin
    ProcedureName := 'up_getBankbookUtilityPayments;1';
    Parameters.Clear;
    Parameters.CreateParameter('@result_value', ftinteger, pdReturnValue, 10, 0);
    Parameters.CreateParameter('@id_bankbook_utility', ftinteger, pdInput, 10, id_bankbook_utility);
    open;
    while not eof do begin
      p_p := TPaymentParams.Create;
      p_p.id_bankbook_utility := id_bankbook_utility;
      p_p.payment_date := FieldByName('date_p').AsDateTime;
      p_p.send_date := FieldByName('date_send').AsDateTime;
      p_p.sum := FieldByName('summ').AsFloat;
      p_p.ward_name := FieldByName('ward_name').AsString;
      p_p.filial := FieldByName('c_fil').AsInteger;
      p_p.station := FieldByName('c_com').AsInteger;
      p_p.file_name_in := FieldByName('file_name_in').AsString;
      p_p.file_name_out := FieldByName('file_name_out').AsString;
      p_p.count_start := FieldByName('countb').AsFloat;
      p_p.count_end := FieldByName('counte').AsFloat;
      p_p.period_begin := FieldByName('dateb').AsDateTime;
      p_p.period_end := FieldByName('datee').AsDateTime;
      l.AddObject(FieldByName('date_p').AsString+' оплачено '+
        FloatToStrF(FieldByName('summ').AsFloat, ffFixed, 5, 2)+' грн.',
        TObject(p_p));
      next;
    end;
    Close;
...
begin
  ...
  with q do begin
    SQL.Clear;
    SQL.Add('SELECT TOP 1 * FROM periods WHERE id_city = '+IntToStr(city_id)+
      ' AND status = 1 ORDER BY date_b DESC');
    Open;
    if not eof then begin
...
begin
  with q do begin
    SQL.Clear;
    SQL.Add('UPDATE firms SET id_template_bo=' +IntToStr(loading_params.rules_template_id)+
      ', id_city_mapping_template='+IntToStr(loading_params.city_template_id)+
      ', id_street_mapping_template='+IntToStr(loading_params.street_template_id)+
      ' WHERE code='+IntToStr(loading_params.firm_id));
    ExecSQL;
  end;
end;
end.
