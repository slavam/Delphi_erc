...
function is_city(ADOQDictionary: TADOQuery; value: string; parent_city_id: integer): boolean;
...
function get_position_mismatch(s1, s2: string): integer;
...
begin
  ...
  with ADOQDictionary do begin
    SQL.Clear;
    SQL.Add('SELECT id_parent FROM cities WHERE id = '+ value);
    Open;
    if (RecordCount > 0) and (parent_city_id = FieldByName('id_parent').AsInteger) then
      result := true;
    Close;
  end
end;
  ...
  with StoredProc do begin
    ...
    if Recordset.RecordCount > 0 then begin
      ...
      for i := 0 to ADODSWork.Recordset.RecordCount - 1 do begin
        s := ANSIUpperCase(AnsiReplaceStr(ADODSWork.FieldByName('full_name').AsString, chr(95), chr(73)));
        if ( s = full_name) or
          (get_position_mismatch(s, full_name) >= pos(' ', s)) then begin
          result := true;
          occupants := '';
          Close;
          exit;
...
begin
  ...
  if (pos(' пер.', s) > 0) or (pos(' пр.', s) > 0) then
    result := result + s+', '
  else
    result := result +'ул. '+s+', ';
...
begin
  ...
  with ADOQDictionary do begin
    ...
    if RecordCount > 0 then begin
      ...
      debt_params.address_id := FieldByName('id_room').AsInteger;
...
begin
  ...
  with ADOQDictionary do begin
    ...
    if RecordCount > 0 then begin
      result := true;
      privilege_params.bankbook_id := FieldByName('id_bankbook').AsInteger;
  ...
  try
    with q do begin
      ...
      for I := 0 to recordCount - 1 do begin
        ...
        payer_params.id_room := FieldByName('id_room').AsInteger;
...
begin
  ...
  with q do begin
    SQL.Clear;
    SQL.Add('SELECT * FROM streets WHERE id_city = '+ IntToStr(id)+'ORDER BY name');
    Open;
    First;
    for I := 0 to recordCount - 1 do begin
      ...
      s_params.id_city := id;
      s_params.code_street := FieldByName('code_s').AsInteger;
      s_params.name_street := FieldByName('name').AsString;
      l.AddObject(FieldByName('name').AsString, s_params);
      next;
    end;
  end;
end;
...
begin
  ...
  with q do begin
    SQL.Clear;
    SQL.Add('SELECT * FROM houses WHERE id_street = '+ IntToStr(id)+
      'ORDER BY n_house, f_house, d_house, a_house');
    Open;
    First;
    for I := 0 to recordCount - 1 do begin
...
begin
  ...
  with q do begin
    SQL.Clear;
    SQL.Add('SELECT * FROM rooms WHERE id_house = '+ IntToStr(id)+
...
begin
  ...
  i := 1;
  while (CompareStr(copy(s1,1,i), copy(s2,1,i))=0) and (i <= min(length(s1), length(s2))) do
    inc(i);
  Result := i;
end;
